...
template < typename real = double , unsigned int _Dim = 3 >
class VectorN {
protected:
    real coord[_Dim]; //!< Where the coordinates are actually stored.
...
public:
    ...
    /// Empty constructor
    VectorN () {
        for (unsigned i = 0; i < _Dim; i++) coord[i] = 0; 
    }
    
    /// Constructor from an array of coordinates
    VectorN (const real v[]) {
        for (unsigned i = 0; i < _Dim; i++) coord[i] = v[i]; 
    }
    
    /// Constructor from another VectorN
    VectorN (const VectorN<real,_Dim>& v) {
        for (unsigned i = 0; i < _Dim; i++) coord[i] = v.coord[i]; 
    }
    ...
    inline real& operator[] (unsigned i) { assert(i<_Dim); return coord[i]; }

    /// \brief Coordinate indexing
    /// \param i  coordinate index.
    /// \return  value of the i'th coordinate value.
    inline real  operator[] (unsigned i) const { assert(i<_Dim); return coord[i]; }

    /// \brief Coordinate indexing
    /// \param i  coordinate index.
    /// \return  value of the i'th coordinate value.
    inline real  at(unsigned i) const { assert(i<_Dim); return coord[i]; }
    ...
    /// \return sum vector
    inline VectorN<real,_Dim> operator+(const VectorN<real,_Dim>& v) const {
        VectorN<real,_Dim> r;
        for (unsigned i = 0; i < _Dim; i++) r.coord[i] = coord[i]+v.coord[i];
        return r;
    }
    ...
    /// \return sum vector
    inline VectorN<real,_Dim> operator+(real alpha) const {
        VectorN<real,_Dim> r;
        for (unsigned i = 0; i < _Dim; i++) r.coord[i] = coord[i]+alpha;
        return r;
    }
    ...
    inline VectorN<real,_Dim> operator*(scalar s) const {
        VectorN<real,_Dim> r;
        for (unsigned i = 0; i < _Dim; i++) r.coord[i] = coord[i] * (real) s;
        return r;
    }
    ...
    /// \param s scalar
    /// \return  product vector
    inline VectorN<real,_Dim> operator/(real s) const { 
        VectorN<real,_Dim> r;
        for (unsigned i = 0; i < _Dim; i++) r.coord[i] = coord[i] / s;
        return r;
    }
    ...
    inline VectorN<real,_Dim> operator-() const {
        VectorN<real,_Dim> r = *this;
        for (unsigned i = 0; i < _Dim; i++) r.coord[i] = -r.coord[i];
        return r;
    } 
    ...
    /// \param v: another vector
    /// \return : dot product
    inline real operator*(const VectorN<real,_Dim>& v) const {
        real sum = 0;
        for (unsigned i = 0; i < _Dim; i++) sum += coord[i]*v.coord[i];
        return sum;
    }
    ...
    /// \param v: another vector
    /// \return : dot product
    inline real dot(const VectorN<real,_Dim>& v) const {
        real sum = 0;
        for (unsigned i = 0; i < _Dim; i++) sum += coord[i]*v.coord[i];
        return sum;
    }
    ...
    inline VectorN<real,_Dim>& operator=(const VectorN<real,_Dim>& v) {
        for (unsigned i = 0; i < _Dim; i++) coord[i] = v.coord[i];
        return *this;
    }
    ...
    inline VectorN<real,_Dim>& operator+=(real alpha) {
        for (unsigned i = 0; i < _Dim; i++) coord[i] += alpha;
        return *this;
    }
    ...
    /// \param s scalar
    /// \return  reference to this (altered) vector
    inline VectorN<real,_Dim>& operator*=(real s) {
        for (unsigned i = 0; i < _Dim; i++) coord[i] *= s;
        return *this;
    }
    ...
    /// \param s scalar
    /// \return reference to this (altered) vector
    inline VectorN<real,_Dim>& operator/=(real s) { return (*this) *= (1/s); }
    ...
    bool operator==(const VectorN<real,_Dim> &v) const {
...
inline VectorN<real,_Dim> operator*(real s, const VectorN<real,_Dim>& v) {
    return v*s;
}
...
template < typename real, unsigned int _Dim >
inline real norm(const VectorN<real,_Dim>& v)
{
    return v.norm();
}
...
template < typename real, unsigned int _Dim >
inline real norm2(const VectorN<real,_Dim>& v)
{
    return v.norm2();
}
...
template < typename real, unsigned int _Dim >
inline real unitize(VectorN<real,_Dim>& v)
{
    double l=norm2(v);
    if( l!=1.0 && l!=0.0 )   {
    	l = sqrt(l);
...
template < typename real, unsigned int _Dim >
inline std::ostream& operator<<(std::ostream& out, const VectorN<real,_Dim>& v)
{
    for (unsigned i = 0; i < _Dim; i++) {
        out << v [i] << " ";
    }
...
template < typename real, unsigned int _Dim >
inline std::istream& operator>>(std::istream& in, VectorN<real,_Dim>& v)
{
    for (unsigned i = 0; i < _Dim; i++) {
        in >> v [i];
    }
...
template < typename real = double , unsigned int _Dim = 3 >
class PointN : public VectorN < real, _Dim > {
public:
    //
    // Standard constructors
    //
    ...
    /// Empty constructor
    PointN () : VectorN<real,_Dim> () {}
    
    /// Constructor from an array of coordinates
    PointN (const real v[]) : VectorN<real,_Dim> (v) {}
    
    /// Constructor from another VectorN
    PointN (const VectorN<real,_Dim>& v) : VectorN<real,_Dim> (v) {}

    /// Constructor from a single coordinate 
    PointN ( real v ) : VectorN<real,_Dim> () {
        for (unsigned int i = 0; i < _Dim; i++) this->coord [i] = v;
    }

    /// Constructor from two coordinates - must be bidimensional
    PointN ( real x , real y) : VectorN<real,_Dim> () {
        assert (_Dim == 2);
        this->coord [0] = x;
        this->coord [1] = y;
    }
    ...
    PointN ( real x,  real y, real z) : VectorN<real,_Dim> () {
        assert (_Dim == 3);
        this->coord [0] = x;
        this->coord [1] = y;
        this->coord [2] = z;
    ...
    /// \param v: a vector
    inline void set( const VectorN<real,_Dim>& v ) { 
        for ( unsigned int i = 0; i < _Dim; i++) this->coord [i] = v[i];
    }
    
    /// Casting to Vector
    inline VectorN<real,_Dim> toVector ( void ) { return VectorN<real,_Dim> (*this); }
...
//    }
    ...
    ///    and p's
    inline PointN<real,_Dim> min(const PointN<real,_Dim> & p) const {
        PointN<real,_Dim> r;
        for ( unsigned int i = 0; i < _Dim; i++) r[i] = minReal(this->at(i),p[i]);
        return r;
    }
    ...
    ///     and p's
    inline PointN<real,_Dim> max(const PointN<real,_Dim> & p) const {
        PointN<real,_Dim> r;
        for ( unsigned int i = 0; i < _Dim; i++) r[i] = maxReal(this->at(i),p[i]);
        return r;
    }
    
    /// Squared distance between two points
    inline real dist2 (const PointN<real,_Dim> & p) const {
        PointN<real,_Dim> q(*this);
        return (p-q).norm2();
    }
    
    /// Euclidian distance between two points
    inline real dist (const PointN<real,_Dim> & p) const {
        return ((*this) - p).norm();
    }
