...
*
**/
template < typename real , unsigned int _Dim , class Point = PointN<real,_Dim> , 
           class Vector = VectorN<real,_Dim> >
...
protected:
    ...
    struct DPitem {
        ...
        DPitem (unsigned f, unsigned l, const PolygonalCurve<real,_Dim,Point,Vector>& poly) 
            : first(f), last(l) 
        {
            assert (last-first > 1); 
...
public:    
    ...
    PolygonalCurve( const PolygonalCurve<real,_Dim,Point,Vector> &curve ) {
    ...
    virtual void copy( const PolygonalCurve<real,_Dim,Point,Vector> &curve ) {
    ...
    /// \param curve: another polygonal curve.
    virtual PolygonalCurve &operator=( const PolygonalCurve<real,_Dim,Point,Vector> &curve ) {
        this->copy(curve);
        return (*this);
    }
    ...
    virtual void addToStart(const Point& p) {
        pPoints.insert(pPoints.begin(),p);
    }
    ...
    virtual PolygonalCurve<real,_Dim,Point,Vector>
                  copyRange( unsigned int idxStart , unsigned int idxEnd ) const
    {
        ...
        if ( p0Idx > this->size() -1 )
                                return PolygonalCurve<real,_Dim,Point,Vector> ();
        p1Idx = ( p1Idx < this->size() ) ? p1Idx + 1 : this->size() ;
        std::vector<Point> points( pPoints.begin()+p0Idx ,
                                   pPoints.begin()+p1Idx ) ;
        PolygonalCurve<real,_Dim,Point,Vector> c(points) ;
        if(revert) c.reverse();
        return c ;
    }
    ...
    inline void catmull(PolygonalCurve<real,_Dim,Point,Vector>& result, real maxdist = 1.0) {
    ...
    void douglasPeuckerSimplify (PolygonalCurve<real,_Dim,Point,Vector>& result, real tol) const {
        assert (size()>0);
    ...
    void douglasPeuckerDecimate (PolygonalCurve<real,_Dim,Point,Vector>& result, int n) const {
        assert (size()>0);
    ...
    virtual void scale(const Vector & v) {
        for( unsigned int i = 0 ; i < size() ; i++ )
            for( unsigned int j = 0 ; j < _Dim ; j++ ) pPoints[i][j]*=v[j];
    }
    ...
    virtual PolygonalCurve<real,_Dim,Point,Vector> split( unsigned int index ) {

        PolygonalCurve<real,_Dim,Point,Vector> newCurve;
    ...
    virtual bool split( unsigned int index0 , unsigned int index1 ,
        PolygonalCurve<real,_Dim,Point,Vector>& c1 ,
        PolygonalCurve<real,_Dim,Point,Vector>& c2 , 
        PolygonalCurve<real,_Dim,Point,Vector>& c3  ) const 
    {
    ...
    virtual bool split( unsigned int index , PolygonalCurve<real,_Dim,Point,Vector>& c1 ,
                        PolygonalCurve<real,_Dim,Point,Vector>& c2 ) const {
        c1 = (*this);
        c2 = c1.split( index );
        return c2.size() > 0;
    }
        ...
        real err = CEPS ) const
    {
        ...
        else if( (index<c1.size()) && ((c1.at(index)-pr).norm() > err && (c1.at(index-1)-pr).norm() > err ))
        {
            ++index ;
    ...
    virtual PolygonalCurve<real,_Dim,Point,Vector> overSketch(
           const PolygonalCurve<real,_Dim,Point,Vector>& curve ,
           PolygonalCurve<real,_Dim,Point,Vector>& rest ,
           real err = CEPS ,
...
//                std::cout << "d0 < approx*err && d1 > approx*error" << std::endl;
        ...
        PolygonalCurve<real,_Dim,Point,Vector> A , B , C , curveTmp0 , curveTmp1  ;
...
typedef PolygonalCurve< double, 3, PointN< double, 3 >, VectorN< double, 3 > > Curve3D;
...
static Curve3D vectorToCurve3D( std::vector< double > source_ )
{
    Curve3D curve_;

    std::size_t number_of_points = static_cast< std::size_t >( source_.size()/3 );
    for( std::size_t i = 0; i < number_of_points; ++i )
    {
        curve_.push_back( Point3D( source_[ 3*i ], source_[ 3*i + 1 ],source_[ 3*i + 2 ] ) );
    }
